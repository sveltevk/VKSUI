<script lang="ts">
	...
	import AppearanceProvider, {
		generateAppearanceClass
	} from '@sveltevk/vksui/components/Service/AppearanceProvider/AppearanceProvider.svelte';
	...
	const toggleApperance = () => (appearance = appearance === 'light' ? 'dark' : 'light');
	...
	$: setBodyClass(generateAppearanceClass($platform, appearance));
		...
		<SplitLayout class="Doc">
			...
			>
				<Panel>
					{#if $adaptivity.viewWidth >= ViewWidth.SMALL_TABLET}
						<PanelHeader>
							<PanelHeaderButton slot="right" on:click={toggleApperance}>
								{#if appearance === 'dark'}
